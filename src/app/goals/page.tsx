...
export default function GoalsPage() {
  const [goals, setGoals] = useState<Array<{ id: string; title: string; targetAmount: number; currentAmount: number }>>([]);
  ...
  async function addSavings(goalId: string) {
    ...
    if (amount <= 0) {
      setResult("Enter a valid amount to add.");
      return;
    }
    ...
    setResult(`Updated goal: ${data.title}`);
    setAddByGoalId((prev) => ({ ...prev, [goalId]: "" }));
    await loadGoals();
  }
