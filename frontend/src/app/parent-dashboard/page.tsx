...
export default function ParentDashboardPage() {
  ...
  return (
    <main className="mx-auto max-w-4xl px-4 py-8">
      ...
      <button onClick={() => void loadDashboard()} className="rounded bg-blue-600 px-4 py-2 font-semibold text-white">Refresh</button>
      ...
      <div className="mt-6 grid gap-4">
        {children.map((child) => (
          <article key={child.id} className="rounded-xl border p-4">
            ...
            {child.recentLessons.length > 0 && <p className="mt-2 text-sm text-zinc-600">Recent lessons: {child.recentLessons.join(", ")}</p>}
          </article>
        ))}
      </div>
    </main>
